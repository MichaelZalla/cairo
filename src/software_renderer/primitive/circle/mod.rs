                ...
                match &framebuffer.attachments.forward_ldr {
                    Some(forward_buffer_rc) => {
                        ...
                        Graphics::circle(
                            ...
                            (radius_ndc_space * self.viewport.width as f32) as u32,
                            fill_u32,
                            border_u32,
                        );
                    }
                    None => panic!(
                        "Called SoftwareRenderer::render_circle_at_ndc_space_position() with no forward (LDR) framebuffer attachment!"
                    ),
                }
            }
            None => panic!(
